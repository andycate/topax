    ...
    def generate_shader_code(graph: Op, prefix: str) -> tuple[list[str], dict[Op, str]]:
        ...
        for ti, op in enumerate(tape):
            ...
            match op.opcode:
                ...
                case OpType.MOD: expression = f"mod({arg_expressions[0]}, {arg_expressions[1]})"
                case OpType.ROUND: expression = f"round({arg_expressions[0]})"
                case OpType.CLAMP: expression = f"clamp({arg_expressions[0]}, {arg_expressions[1]}, {arg_expressions[2]})"
